...
void print_usage(const char* prog_name) {
    fprintf(stderr, "Usage: %s <input.vgm> [-o <output.txt>]\n", prog_name);
}
...
int main(int argc, char* argv[]) {
    ...
    fprintf(out_fp, "Offset(h) | Command\n");
    fprintf(out_fp, "----------|------------------------------------------------\n");
    ...
    while (fread(&cmd_buf[0], 1, 1, in_fp) == 1) {
        ...
        fprintf(out_fp, "%08lX  | %02X: %s", current_pos, op, info->name);

        if (data_len > 0) {
            if (fread(&cmd_buf[1], 1, data_len, in_fp) != data_len) {
                fprintf(out_fp, " ... (incomplete command)\n");
                break;
            }
        }

        // Special handling for printing details
        switch (op) {
            case 0x61: // WAIT nn nn
                fprintf(out_fp, " (val: %u)", read_le16(&cmd_buf[1]));
                break;
            case 0x62: // WAIT 735
                fprintf(out_fp, " (735 samples)");
                break;
            case 0x63: // WAIT 882
                fprintf(out_fp, " (882 samples)");
                break;
            case 0x66: // END
                fprintf(out_fp, "\n");
                goto end_loop;
            case 0x67: { // DATA_BLOCK
                fseek(in_fp, 2, SEEK_CUR); // Skip 0x66 type
                uint32_t block_size = 0;
                fread(&block_size, 4, 1, in_fp);
                fprintf(out_fp, " (type: 0x%02X, size: %u)", cmd_buf[1], block_size);
                fseek(in_fp, block_size, SEEK_CUR);
                current_pos += 6 + block_size;
                data_len = 0; // Already handled
                break;
            }
            case 0x50: // SN76489
            case 0x30: // SN76489 #2
                fprintf(out_fp, " (val: 0x%02X)", cmd_buf[1]);
                break;
            case 0xA0: // AY8910
            case 0x51: // YM2413
            case 0x52: // YM2612 P0
            ...
            case 0xBC: // WonderSwan
                fprintf(out_fp, " (reg: 0x%02X, val: 0x%02X)", cmd_buf[1], cmd_buf[2]);
                break;
            default:
                if (op >= 0x70 && op <= 0x7F) {
                    fprintf(out_fp, " (val: %u)", (op & 0x0F) + 1);
                } else {
