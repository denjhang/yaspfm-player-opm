    ...
    while (!g_quit_flag) {
        if (_kbhit()) {
            ...
            switch (c) {
                ...
                case 'r': g_play_mode = (g_play_mode == PLAY_MODE_SEQUENTIAL) ? PLAY_MODE_RANDOM : PLAY_MODE_SEQUENTIAL; g_ui_refresh_request = true; break;
    ...
    while (!g_quit_flag) {
        ...
        if (should_play) {
            ...
            if (g_quit_flag) {
                ...
                // Flags are already set, the main loop will handle it.
                // Just stop the 'current' play session.
                g_is_playing = false;
            } else {
    ...
    while (!g_quit_flag) {
        if (g_app_mode == MODE_PLAYER) {
            if (g_ui_refresh_request) {
                update_ui(g_current_song_total_samples, g_current_song_name, g_is_paused, g_play_mode);
