...
void opn_to_opm_init(chip_type_t source_chip_type, uint32_t source_clock, opm_write_func_t write_func) {
    ...
    memset(_lr_cache, 0, sizeof(_lr_cache));
    
    // Initial commands from vgm-conv-main
    if (_write_func) {
        _write_func(0x18, 0xc2); // LFO FREQ
    }
}
...
void opn_to_opm_write_reg(uint8_t addr, uint8_t data, uint8_t port) {
    ...
    if (port == 0) {
        if (addr == 0x22 && _source_chip != CHIP_TYPE_YM2203) {
            const uint8_t lfreq[] = {0xc2, 0xc9, 0xcb, 0xcd, 0xcf, 0xd6, 0xfb, 0xff};
            _write_func(0x18, lfreq[data & 7]);
        } else if (addr == 0x28) {
            uint8_t ch = data & 0x07;
            uint8_t slots = (data & 0xf0) >> 4;
            _write_func(0x08, (slots << 3) | ch);
        }
    }

    if (addr >= 0x30 && addr <= 0x8f) {
        uint8_t nch = addr & 3;
        uint8_t ch = (port == 0 ? 0 : 4) + nch;
