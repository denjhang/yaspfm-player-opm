...
#ifdef _WIN32
    if (strcmp(current_path, ".") == 0 && strchr(current_path, ':') == NULL) {
...
const char* file_browser(const char* initial_path) {
    ...
    while (1) {
        if (_kbhit()) {
                ...
                case 13: // Enter
                    if (file_list[selected_index].is_dir) {
                        if (is_drive_list) {
                            ...
                            if (last_slash == current_path + strlen(current_path) - 1) { // trailing slash
                                *last_slash = '\0';
                                last_slash = strrchr(current_path, '/');
                            }
                            
                            if (last_slash) {
                                if (strchr(current_path, ':') && last_slash == strchr(current_path, ':') + 1) {
                                     *(last_slash + 1) = '\0'; // Keep "C:/"
                                } else {
                                    *last_slash = '\0';
                                }
                            } else {
                        ...
                        logging(DEBUG, "File selected in browser: %s", selected_file_path);
            ...
            yasp_usleep(50000); // 50ms sleep to prevent busy-waiting
        }
    }
    return NULL;
}
