...
void update_ui(uint32_t total_samples, const char* song_name, bool paused, int play_mode) {
    ...
    print_at(0, 1, "--------------------------------------------------");
    print_at(0, 15, "--------------------------------------------------");
    print_at(0, 16, "[N] Next | [B] Prev | [P] Pause | [R] Random | [F] Browser | [+] Speed Up | [-] Speed Down | [Q] Quit");
    print_at(0, 17, "--------------------------------------------------");
    ...
    // --- Show VGM Chip Info ---
    uint32_t clock = 0;
    uint32_t default_clock = get_chip_default_clock(g_vgm_chip_type);

    if (g_vgm_chip_type == CHIP_TYPE_YM2612) clock = g_vgm_header.ym2612_clock;
    else if (g_vgm_chip_type == CHIP_TYPE_YM2203) clock = g_vgm_header.ym2203_clock;
    else if (g_vgm_chip_type == CHIP_TYPE_YM2608) clock = g_vgm_header.ym2608_clock;
    else if (g_vgm_chip_type == CHIP_TYPE_YM2151) clock = g_vgm_header.ym2151_clock;
    else if (g_vgm_chip_type == CHIP_TYPE_AY8910) clock = g_vgm_header.ay8910_clock;
    // Add other chips here

    if (clock == 0) {
        snprintf(buffer, sizeof(buffer), "VGM Chip: %s (%.2fMHz, inferred)", chip_type_to_string(g_vgm_chip_type), default_clock / 1000000.0);
    } else {
        snprintf(buffer, sizeof(buffer), "VGM Chip: %s (%.2fMHz)", chip_type_to_string(g_vgm_chip_type), clock / 1000000.0);
    }
    ...
    } else if (g_opn_to_opm_conversion_enabled) {
        uint32_t conv_clock = 0;
        if (g_vgm_chip_type == CHIP_TYPE_YM2612) conv_clock = g_vgm_header.ym2612_clock;
        else if (g_vgm_chip_type == CHIP_TYPE_YM2203) conv_clock = g_vgm_header.ym2203_clock;
        else if (g_vgm_chip_type == CHIP_TYPE_YM2608) conv_clock = g_vgm_header.ym2608_clock;
        
        if (conv_clock == 0) {
            conv_clock = get_chip_default_clock(g_vgm_chip_type);
        }

        snprintf(buffer, sizeof(buffer), "Conversion: %s (%.2fMHz) -> YM2151 (%.2fMHz)", 
            chip_type_to_string(g_vgm_chip_type), 
            conv_clock / 1000000.0,
            get_chip_default_clock(CHIP_TYPE_YM2151) / 1000000.0);
        clear_line(15); print_at(0, 15, buffer);
    } else if (g_ay_to_opm_conversion_enabled) {
        uint32_t conv_clock = g_vgm_header.ay8910_clock;
        if (conv_clock == 0) {
            conv_clock = get_chip_default_clock(g_vgm_chip_type);
        }
        snprintf(buffer, sizeof(buffer), "Conversion: %s (%.2fMHz) -> YM2151 (%.2fMHz)", 
...
bool play_file(const char *filename) {
    logging(DEBUG, "Attempting to play file: %s", filename);
    ...
    switch (type) {
        case FILETYPE_VGM:
            logging(DEBUG, "Calling vgm_play for %s", filename);
            vgm_play(fp, filename, cache_filename);
            result = true; // Assume success for now
            break;
        case FILETYPE_S98: {
            S98 s98 = {0};
            if (s98_load(&s98, fp)) {
                logging(DEBUG, "Calling s98_play for %s", filename);
