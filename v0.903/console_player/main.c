...
void clear_cache(const char* cache_path) {
    ...
    if (!(dir = opendir(cache_path))) {
        logging(LOG_ERROR, "Could not open cache directory: %s", cache_path);
        return;
    }

    logging(INFO, "Clearing cache directory: %s", cache_path);
    while ((entry = readdir(dir)) != NULL) {
        ...
        if (remove(filepath) == 0) {
            ...
            logging(LOG_ERROR, "Failed to remove: %s", filepath);
        }
    }
    closedir(dir);
...
int main(int argc, char *argv[]) {
    ...
    if (config.last_file[0] != '\0') {
        ...
        if (f) {
            ...
            logging(INFO, "Last played file '%s' not found, rescanning.", config.last_file);
    ...
    if (!last_file_valid) {
        ...
        if (playlist_size == 0) {
            ...
            logging(INFO, "No music in 'music' folder, scanning current directory.");
    ...
    if (playlist_size == 0) {
        logging(INFO, "No music found, starting file browser.");
