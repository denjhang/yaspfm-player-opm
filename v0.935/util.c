...
// Global key queue implementation for unified keyboard handling
#define KEY_QUEUE_SIZE 128

static char g_key_queue[KEY_QUEUE_SIZE];
static int g_key_queue_head = 0;
static int g_key_queue_tail = 0;

#ifdef _WIN32
static CRITICAL_SECTION g_key_queue_lock;
#else
static pthread_mutex_t g_key_queue_lock = PTHREAD_MUTEX_INITIALIZER;
#endif

void key_queue_init(void) {
#ifdef _WIN32
    InitializeCriticalSection(&g_key_queue_lock);
#else
    pthread_mutex_init(&g_key_queue_lock, NULL);
#endif
    g_key_queue_head = 0;
    g_key_queue_tail = 0;
}

void key_queue_cleanup(void) {
#ifdef _WIN32
    DeleteCriticalSection(&g_key_queue_lock);
#else
    pthread_mutex_destroy(&g_key_queue_lock);
#endif
}

void key_queue_enqueue(char key) {
#ifdef _WIN32
    EnterCriticalSection(&g_key_queue_lock);
#else
    pthread_mutex_lock(&g_key_queue_lock);
#endif

    int next_head = (g_key_queue_head + 1) % KEY_QUEUE_SIZE;
...
#ifdef _WIN32
    LeaveCriticalSection(&g_key_queue_lock);
#else
    pthread_mutex_unlock(&g_key_queue_lock);
#endif
}

bool key_queue_dequeue(char* key) {
#ifdef _WIN32
    EnterCriticalSection(&g_key_queue_lock);
#else
    pthread_mutex_lock(&g_key_queue_lock);
