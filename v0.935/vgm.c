...
bool g_ws_to_opm_conversion_enabled = false;
...
uint32_t g_original_vgm_chip_clock = 0; // To store the clock of the original chip
...
static void vgm_analyze_and_set_ssg_mapping(const uint8_t* vgm_data, size_t vgm_data_size);
...
bool vgm_parse_header(FILE* fp, vgm_header_t* header) {
    ...
    if (fseek(fp, 0, SEEK_SET) != 0 || fread(hdr_buf, 1, 0x40, fp) != 0x40) {
        logging(LOG_LEVEL_ERROR, "Could not read first 0x40 bytes of header.\n");
        return false;
    }

    if (memcmp(hdr_buf, "Vgm ", 4) != 0) {
        logging(LOG_LEVEL_ERROR, "Invalid VGM signature.\n");
        return false;
    }
    ...
    if (header_size_to_read > 0x40) {
        if (fread(hdr_buf + 0x40, 1, header_size_to_read - 0x40, fp) != (header_size_to_read - 0x40)) {
            logging(LOG_LEVEL_WARN, "Could not read full extended header. Some data may be missing.\n");
        }
    }
    ...
    if (header->version >= 0x151) {
        ...
        header->ym3526_clock = read_le32(hdr_buf + 0x4C);
        header->y8950_clock = read_le32(hdr_buf + 0x54);
...
int vgm_process_command(FILE *input_fp, int* vgm_wait1, int* vgm_wait2, int* loop_counter) {
    ...
    switch (op) {
        ...
        case 0x54:
            ...
            break;
        case 0x5C: // Y8950 (正确的VGM命令)
            if (fread(buf, 1, 2, input_fp) != 2) { g_is_playing = false; return 0; }
            y8950_write_reg(get_slot_for_chip(CHIP_TYPE_Y8950), buf[0], buf[1]);
            break;
        case 0xAC: // Y8950 第2个芯片
            if (fread(buf, 1, 2, input_fp) != 2) { g_is_playing = false; return 0; }
            {
                uint8_t slot = get_slot_for_chip_at_index(CHIP_TYPE_Y8950, 1);
                if (slot != 0xFF) {
                    y8950_write_reg(slot, buf[0], buf[1]);
                }
            }
            break;
        case 0x55:
            if (fread(buf, 1, 2, input_fp) != 2) { g_is_playing = false; return 0; }
            if (g_opn_to_opm_conversion_enabled && g_vgm_chip_type == CHIP_TYPE_YM2203) {
                if (buf[0] < 0x10) { // SSG part
                    ssg_to_opm_write_reg(buf[0], buf[1]);
                } else { // FM part
                    opn_to_opm_write_reg(buf[0], buf[1], 0);
                }
            } else {
        ...
        case 0x56: case 0x57:
            if (fread(buf, 1, 2, input_fp) != 2) { g_is_playing = false; return 0; }
            if (g_opn_to_opm_conversion_enabled && g_vgm_chip_type == CHIP_TYPE_YM2608) {
                uint8_t port = (op == 0x56) ? 0 : 1;
                // SSG registers (00-0F) are accessible via both ports.
                if (buf[0] < 0x10) { // SSG part
                    ssg_to_opm_write_reg(buf[0], buf[1]);
                } else { // FM or ADPCM part
                    opn_to_opm_write_reg(buf[0], buf[1], port);
                }
            } else {
        ...
        {
            uint8_t addr, data;
            if (fread(&addr, 1, 1, input_fp) != 1) { g_is_playing = false; return 0; }
            if (fread(&data, 1, 1, input_fp) != 1) { g_is_playing = false; return 0; }
            if (g_ws_to_opm_conversion_enabled) ws_to_opm_write_reg(0, addr, data);
        }
        break;
    case 0xBD: // SAA1099 (both chips)
        {
            uint8_t addr, data;
            if (fread(&addr, 1, 1, input_fp) != 1) { g_is_playing = false; return 0; }
            if (fread(&data, 1, 1, input_fp) != 1) { g_is_playing = false; return 0; }
            // Bit 7 of addr selects chip: 0=Chip 0, 1=Chip 1 (per libvgm implementation)
            uint8_t chip_id = (addr & 0x80) >> 7;
            uint8_t reg_addr = addr & 0x7F;  // Actual register address is bits 0-6
            uint8_t slot = get_slot_for_chip_at_index(CHIP_TYPE_SAA1099, chip_id);
            // Note: Chip 1 commands are silently ignored if Chip 1 doesn't exist
            // This is intentional - dual-chip VGMs will only play Chip 0 part
            if (slot != 0xFF) {
                saa1099_write_reg(slot, reg_addr, data);
            }
        }
        break;
        case 0x61:
        ...
        case 0x67: // Data block
        {
            ...
            // 跳过0x66兼容性字节
            if (fread(&type, 1, 1, input_fp) != 1) { g_is_playing = false; return 0; }
            if (type != 0x66) { g_is_playing = false; return 0; }

            // 读取数据块类型
            if (fread(&type, 1, 1, input_fp) != 1) { g_is_playing = false; return 0; }

            // 读取数据块大小
            if (fread(&size, 4, 1, input_fp) != 1) { g_is_playing = false; return 0; }

            // 处理Y8950 ADPCM数据块(类型0x88)
            if (type == 0x88) {
                uint32_t rom_size, start_addr;

                // 读取ROM大小
                if (fread(&rom_size, 4, 1, input_fp) != 1) { g_is_playing = false; return 0; }

                // 读取起始地址
                if (fread(&start_addr, 4, 1, input_fp) != 1) { g_is_playing = false; return 0; }

                // 计算实际数据大小（减去8字节的头部）
                uint32_t data_size = size - 8;

                // 分配临时缓冲区读取数据
                uint8_t* adpcm_data = (uint8_t*)malloc(data_size);
                if (adpcm_data != NULL) {
                    if (fread(adpcm_data, 1, data_size, input_fp) == data_size) {
                        // 写入Y8950 ADPCM数据，并获取消耗的等待样本数
                        uint8_t slot = get_slot_for_chip(CHIP_TYPE_Y8950);
                        if (slot != 0xFF) {
                            wait_samples += y8950_write_adpcm_data(slot, rom_size, start_addr, adpcm_data, data_size);
                        }
                    }
                    free(adpcm_data);
                } else {
                    // 内存分配失败，跳过数据
                    fseek(input_fp, data_size, SEEK_CUR);
                }
            } else {
                // 其他类型的数据块，跳过
                fseek(input_fp, size, SEEK_CUR);
            }
            break;
        }
        default:
...
FILE* vgm_play(FILE *input_fp, const char *filename, const char *cache_filename, bool force_reconvert) {
    ...
    g_original_vgm_chip_clock = get_clock_from_header(&g_vgm_header, g_original_vgm_chip_type);
    ...
    g_ws_to_opm_conversion_enabled = false;
    g_is_conversion_active = false;

    // Only convert if native hardware is NOT available
    bool ay_available = (get_slot_for_chip(CHIP_TYPE_AY8910) != 0xFF);
    bool sn_available = (get_slot_for_chip(CHIP_TYPE_SN76489) != 0xFF);

    bool needs_ay_conversion = (g_vgm_chip_type == CHIP_TYPE_AY8910) && !ay_available;
    bool needs_sn_conversion = (g_vgm_chip_type == CHIP_TYPE_SN76489) && !sn_available;
    bool needs_ws_conversion = (g_vgm_chip_type == CHIP_TYPE_WSWAN);  // WSWAN has no native hardware

    if ((needs_opn_conversion || needs_ay_conversion || needs_sn_conversion || needs_ws_conversion) && opm_available) {
        ...
        if (needs_ws_conversion) g_ws_to_opm_conversion_enabled = true;
        ...
        if (cache_fp_read) {
            // --- CACHE EXISTS ---
            logging(LOG_LEVEL_INFO, "Found cache file: %s. Playing from cache.", cache_filename);
            ...
            logging(LOG_LEVEL_INFO, "Cache not found. Converting %s to OPM...", chip_type_to_string(g_vgm_chip_type));
            ...
            if (!original_file_data || fread(original_file_data, 1, original_file_size, input_fp) != (size_t)original_file_size) {
                logging(LOG_LEVEL_ERROR, "Failed to read original VGM file into memory.");
            ...
            if (!g_cache_fp) {
                logging(LOG_LEVEL_ERROR, "Failed to open cache file for writing: %s", cache_filename);
            ...
            if (!current_fp) { 
                logging(LOG_LEVEL_ERROR, "Failed to open newly created cache file!"); 
...
DWORD WINAPI vgm_player_thread(LPVOID lpParam) {
    ...
    if (g_timer_mode == 3 || g_timer_mode == 7) {
        ...
        if (timer_id == 0) {
            logging(LOG_LEVEL_ERROR, "Failed to create multimedia timer for VGMPlay mode.\n");
...
static bool vgm_convert_and_cache_from_mem(const uint8_t* vgm_data, size_t vgm_data_size, const vgm_header_t* original_header) {
    ...
    if (g_opn_to_opm_conversion_enabled) {
        ...
        if (original_chip_type == CHIP_TYPE_YM2203 || original_chip_type == CHIP_TYPE_YM2608) {
            ssg_to_opm_init(original_chip_type, original_clock, vgm_cache_opm_writer);
            if (original_chip_type == CHIP_TYPE_YM2608) {
                vgm_analyze_and_set_ssg_mapping(vgm_data, vgm_data_size);
            }
        }
    } else if (g_ay_to_opm_conversion_enabled) {
        ay_to_opm_init(original_chip_type, original_clock, vgm_cache_opm_writer);
    } else if (g_ws_to_opm_conversion_enabled) {
        ws_to_opm_init(original_chip_type, original_clock, vgm_cache_opm_writer);
    ...
    while (pos < vgm_data_size) {
        ...
        switch (op) {
            ...
            case 0x56: case 0x57: { // YM2608
                ...
                if (g_opn_to_opm_conversion_enabled && original_chip_type == CHIP_TYPE_YM2608) {
                    ...
                    if (d1 < 0x10) { // SSG part
                        ...
                        opn_to_opm_write_reg(d1, d2, port);
                    }
                } else {
                    fputc(op, g_cache_fp); fputc(d1, g_cache_fp); fputc(d2, g_cache_fp);
                }
            ...
            case 0xA0: { // AY8910
                ...
                } else if (!g_is_conversion_active) { // Passthrough only if no conversion is active
                    fputc(op, g_cache_fp); fputc(d1, g_cache_fp); fputc(d2, g_cache_fp);
                }
                // Otherwise, drop the command if we are converting another chip to OPM
                break;
            }
            case 0x50: { // SN76489
            ...
            case 0xBC: { // WonderSwan - Format: BC aa dd
                if (pos + 2 > vgm_data_size) goto end_convert_loop;
                uint8_t addr = vgm_data[pos++];
                uint8_t data = vgm_data[pos++];
                if (g_ws_to_opm_conversion_enabled && original_chip_type == CHIP_TYPE_WSWAN) {
                    ws_to_opm_write_reg(0, addr, data);
                } else {
                    // Passthrough if not converting
                    fputc(op, g_cache_fp);
                    fputc(addr, g_cache_fp);
                    fputc(data, g_cache_fp);
                }
            ...
            // --- Non-Convertible Chips (Drop if conversion is active) ---
            case 0x54: // YM2151
            case 0x51: // YM2413
            case 0x58: case 0x59: case 0x5A: case 0x5B: // YM3812, YM3526, YM3812(2nd), YMZ280B
            case 0x5C: case 0x5D: case 0x5E: case 0x5F: // Y8950, YMF278B, YMF271, YMF262
            case 0xAC: // Y8950 Chip 1 (2nd chip)
            case 0xBD: // SAA1099 Chip 0
            case 0xBE: // SAA1099 Chip 1
                if (pos + 2 > vgm_data_size) goto end_convert_loop;
                d1 = vgm_data[pos++]; d2 = vgm_data[pos++];
                // SAA1099 通常不参与 OPM 转换，直接透传
                fputc(op, g_cache_fp);
                fputc(d1, g_cache_fp);
                fputc(d2, g_cache_fp);
                break;
            case 0xB0: case 0xB1: case 0xB2: case 0xB3: // RF5C68, RF5C164, PWM, GB DMG
            case 0xB4: case 0xB5: case 0xB6: case 0xB7: // NES APU, MultiPCM, uPD7759, OKIM6258
            case 0xB8: // OKIM6295
                if (pos + 2 > vgm_data_size) goto end_convert_loop;
                if (!g_is_conversion_active) {
                    d1 = vgm_data[pos]; d2 = vgm_data[pos+1];
                    fputc(op, g_cache_fp); fputc(d1, g_cache_fp); fputc(d2, g_cache_fp);
                }
                pos += 2;
                break;
            
            case 0xC0: case 0xC1: case 0xC2: // SegaPCM, RF5C68, RF5C164 mem write
            case 0xD0: case 0xD1: case 0xD2: // YMF278B, YMF271, SCC1
                if (pos + 3 > vgm_data_size) goto end_convert_loop;
                if (!g_is_conversion_active) {
                    fwrite(&vgm_data[pos-1], 1, 4, g_cache_fp);
                }
                pos += 3;
                break;

            case 0x4F: // GG Stereo
            case 0x95: // DAC Stream Stop
                if (pos + 1 > vgm_data_size) goto end_convert_loop;
                if (!g_is_conversion_active) {
                    fwrite(&vgm_data[pos-1], 1, 2, g_cache_fp);
                }
                pos += 1;
                break;

            case 0x90: case 0x91: case 0x92: case 0x93: case 0x94: // DAC Stream Control
                // These are complex and should be dropped during conversion
                if (op == 0x90 && pos + 4 <= vgm_data_size) pos += 4;
                else if (op == 0x91 && pos + 4 <= vgm_data_size) pos += 4;
                else if (op == 0x92 && pos + 5 <= vgm_data_size) pos += 5;
                else if (op == 0x93 && pos + 10 <= vgm_data_size) pos += 10;
                else if (op == 0x94 && pos + 1 <= vgm_data_size) pos += 1;
                else goto end_convert_loop; // Invalid format
                break;
            ...
            case 0x61: {
                ...
                pos += 2;
                if (g_opn_to_opm_conversion_enabled && (original_chip_type == CHIP_TYPE_YM2203 || original_chip_type == CHIP_TYPE_YM2608)) {
                    for (int i = 0; i < wait; i++) {
                        ssg_to_opm_update_envelope();
                    }
                }
                ...
                if (g_ws_to_opm_conversion_enabled) {
                    ws_to_opm_update(wait);
                }
                fputc(op, g_cache_fp);
            ...
            case 0x67: { // Data block
                ...
                // Skip the command byte (0x67), the type byte, and the 4-byte size
            ...
            // --- Special Handling ---
            default:
                // During conversion, drop all unknown or unhandled commands.
                if (g_is_conversion_active) {
                    // The only commands we let through are waits and end.
                    // All other commands are assumed to be for chips we are replacing.
                    // We just need to advance the position pointer correctly.
                    if (op >= 0x30 && op <= 0x3F) pos += 1;
                    else if (op >= 0x40 && op <= 0x4E) pos += 2;
                    else if (op >= 0xA1 && op <= 0xBF) pos += 2;
                    else if (op >= 0xC0 && op <= 0xCF) pos += 3;
                    else if (op >= 0xD0 && op <= 0xDF) pos += 3;
                    else if (op >= 0xE1) pos += 4;
                    // Let the main loop handle the pos++ for single-byte commands.
                }

                if (op >= 0x70 && op <= 0x7F) { // Wait n+1 samples
                    uint16_t wait = (op & 0x0F) + 1;
                    if (g_opn_to_opm_conversion_enabled && (original_chip_type == CHIP_TYPE_YM2203 || original_chip_type == CHIP_TYPE_YM2608)) {
                        for (int i = 0; i < wait; i++) {
                            ssg_to_opm_update_envelope();
                        }
                    ...
                    // During conversion, we drop the PCM write and just keep the wait.
                    uint8_t n_wait = op & 0x0F;
                    if (n_wait > 0) {
                        if (g_opn_to_opm_conversion_enabled && (original_chip_type == CHIP_TYPE_YM2203 || original_chip_type == CHIP_TYPE_YM2608)) {
                            for (int i = 0; i < n_wait; i++) ssg_to_opm_update_envelope();
                        }
                        if (g_ay_to_opm_conversion_enabled) {
                            for (int i = 0; i < n_wait; i++) ay_to_opm_update_envelope();
                        }
                        if (g_ws_to_opm_conversion_enabled) {
                            ws_to_opm_update(n_wait);
                        }
                        fputc(0x70 | (n_wait - 1), g_cache_fp);
                    }
                    pos++; // Skip the data byte for the PCM write.
                }
                // Any other unknown command is simply dropped if conversion is active.
                break;
        }
    }
end_convert_loop:;
...
// Pre-scans VGM data to determine channel usage for YM2608 conversion
static void vgm_analyze_and_set_ssg_mapping(const uint8_t* vgm_data, size_t vgm_data_size) {
    uint32_t ssg_note_counts[3] = {0, 0, 0};
    bool ssg_noise_usage[3] = {false, false, false};
    bool fm_channel_used[6] = {false, false, false, false, false, false};

    size_t pos = 0;
    while (pos < vgm_data_size) {
        uint8_t op = vgm_data[pos++];
        switch (op) {
            case 0x56: case 0x57: { // YM2608 write
                if (pos + 2 > vgm_data_size) break;
                uint8_t addr = vgm_data[pos];
                uint8_t data = vgm_data[pos + 1];

                if (addr < 0x10) { // SSG part
                    if (addr <= 0x05) ssg_note_counts[addr / 2]++;
                    else if (addr == 0x07) {
                        for (int i = 0; i < 3; i++) if (!((data >> (i + 3)) & 1)) ssg_noise_usage[i] = true;
                    }
                } else if (addr == 0x28) { // FM Key On/Off is a reliable indicator of channel use
                    uint8_t port = (op == 0x56) ? 0 : 1;
                    int ch = data & 0x07;
                    if (port == 1) ch += 3; // Port 1 controls channels 4-6 (indices 3-5)
                    if (ch < 6) fm_channel_used[ch] = true;
                }
                pos += 2;
                break;
            }
            // Skip other commands with arguments
            case 0x61: case 0x52: case 0x53: case 0x54: case 0x55:
            case 0x5C: // Y8950
            case 0xA0: case 0xAC: // AY8910, Y8950 Chip 1
            case 0xBC: // WonderSwan
                pos += 2;
                break;
            case 0x50: pos += 1; break;
            case 0x67: {
                if (pos + 6 > vgm_data_size) { pos = vgm_data_size; break; }
                uint32_t block_size = read_le32(&vgm_data[pos + 2]);
                pos += 6 + block_size;
                break;
            }
            case 0xE0: pos += 4; break;
            default: break; // Single byte commands
        }
    }

    // --- Channel Selection Logic ---
    // New priority: Start with OPM channels 7 and 6, then check for free 5 and 4.
    int available_opm_channels[4] = {-1, -1, -1, -1}; // Can have up to 4 channels
    int opm_ch_idx = 0;

    // Priority 1: Channel 7 (always available for SSG)
    available_opm_channels[opm_ch_idx++] = 7;
    
    // Priority 2: Channel 6 (always available for SSG)
    available_opm_channels[opm_ch_idx++] = 6;

    // Priority 3: Channel 5 (if not used by FM)
    if (opm_ch_idx < 4 && !fm_channel_used[5]) {
        available_opm_channels[opm_ch_idx++] = 5;
    }

    // Priority 4: Channel 4 (if not used by FM)
    if (opm_ch_idx < 4 && !fm_channel_used[4]) {
        available_opm_channels[opm_ch_idx++] = 4;
    }

    int ssg_scores[3] = {0, 0, 0};
    for (int i = 0; i < 3; i++) {
        ssg_scores[i] = ssg_note_counts[i] + (ssg_noise_usage[i] ? 1000000 : 0);
    }

    // Simple bubble sort for SSG channels by score
    int ssg_ch_priority[3] = {0, 1, 2};
    for (int i = 0; i < 2; i++) {
        for (int j = 0; j < 2 - i; j++) {
            if (ssg_scores[ssg_ch_priority[j]] < ssg_scores[ssg_ch_priority[j+1]]) {
                int temp = ssg_ch_priority[j];
                ssg_ch_priority[j] = ssg_ch_priority[j+1];
                ssg_ch_priority[j+1] = temp;
            }
        }
    }

    logging(LOG_LEVEL_INFO, "SSG Analysis: Ch0(S:%d), Ch1(S:%d), Ch2(S:%d)", ssg_scores[0], ssg_scores[1], ssg_scores[2]);
    logging(LOG_LEVEL_INFO, "FM Usage: %d,%d,%d,%d,%d,%d", fm_channel_used[0],fm_channel_used[1],fm_channel_used[2],fm_channel_used[3],fm_channel_used[4],fm_channel_used[5]);
    
    int ssg_ch_map[3] = {-1, -1, -1};
    int opm_ch_map[3] = {-1, -1, -1};
    int mapped_count = 0;

    // How many SSG channels can we map?
    int num_available = 0;
    for(int i=0; i<3; i++) if(available_opm_channels[i] != -1) num_available++;

    for (int i = 0; i < num_available && i < 3; i++) {
        ssg_ch_map[i] = ssg_ch_priority[i];
        opm_ch_map[i] = available_opm_channels[i];
        mapped_count++;
    }

    logging(LOG_LEVEL_INFO, "Mapping %d SSG channels.", mapped_count);
    if(mapped_count > 0) logging(LOG_LEVEL_INFO, "SSG %d -> OPM %d", ssg_ch_map[0], opm_ch_map[0]);
    if(mapped_count > 1) logging(LOG_LEVEL_INFO, "SSG %d -> OPM %d", ssg_ch_map[1], opm_ch_map[1]);
    if(mapped_count > 2) logging(LOG_LEVEL_INFO, "SSG %d -> OPM %d", ssg_ch_map[2], opm_ch_map[2]);

    ssg_to_opm_set_fixed_mapping(ssg_ch_map, opm_ch_map, mapped_count);
}

static chip_type_t get_primary_chip_from_header(const vgm_header_t* header) {
    // 优先检测SAA1099，因为它是较新的芯片，可能被其他芯片覆盖
    // 注意：需要屏蔽双芯片标志位(bit 30 = 0x40000000)来正确检测
    if ((header->saa1099_clock & 0x3FFFFFFF) > 0) return CHIP_TYPE_SAA1099;
    if ((header->ym2608_clock & 0x3FFFFFFF) > 0) return CHIP_TYPE_YM2608;
    if ((header->ym2612_clock & 0x3FFFFFFF) > 0) return CHIP_TYPE_YM2612;
    if ((header->ym2203_clock & 0x3FFFFFFF) > 0) return CHIP_TYPE_YM2203;
    if ((header->ym2151_clock & 0x3FFFFFFF) > 0) return CHIP_TYPE_YM2151;
    if ((header->ay8910_clock & 0x3FFFFFFF) > 0) return CHIP_TYPE_AY8910;
    if ((header->sn76489_clock & 0x3FFFFFFF) > 0) return CHIP_TYPE_SN76489;
    if ((header->y8950_clock & 0x3FFFFFFF) > 0) return CHIP_TYPE_Y8950;
    if ((header->ym2413_clock & 0x3FFFFFFF) > 0) return CHIP_TYPE_YM2413;
    if ((header->wonderswan_clock & 0x3FFFFFFF) > 0) return CHIP_TYPE_WSWAN;
    if ((header->ym3812_clock & 0x3FFFFFFF) > 0) return CHIP_TYPE_YM3812;
    if ((header->ym3526_clock & 0x3FFFFFFF) > 0) return CHIP_TYPE_YM3526;
    return CHIP_TYPE_NONE;
}

static uint32_t get_clock_from_header(const vgm_header_t* header, chip_type_t chip_type) {
    switch (chip_type) {
        case CHIP_TYPE_WSWAN: return header->wonderswan_clock & 0x3FFFFFFF;
        case CHIP_TYPE_YM2608: return header->ym2608_clock & 0x3FFFFFFF;
        case CHIP_TYPE_YM2612: return header->ym2612_clock & 0x3FFFFFFF;
        case CHIP_TYPE_YM2203: return header->ym2203_clock & 0x3FFFFFFF;
        case CHIP_TYPE_YM2151: return header->ym2151_clock & 0x3FFFFFFF;
        case CHIP_TYPE_AY8910: return header->ay8910_clock & 0x3FFFFFFF;
        case CHIP_TYPE_SN76489: return header->sn76489_clock & 0x3FFFFFFF;
        case CHIP_TYPE_SAA1099: return header->saa1099_clock & 0x3FFFFFFF;
        case CHIP_TYPE_YM2413: return header->ym2413_clock & 0x3FFFFFFF;
        case CHIP_TYPE_Y8950: return header->y8950_clock & 0x3FFFFFFF;
        case CHIP_TYPE_YM3812: return header->ym3812_clock & 0x3FFFFFFF;
        case CHIP_TYPE_YM3526: return header->ym3526_clock & 0x3FFFFFFF;
        default: return 0;
    }
}
