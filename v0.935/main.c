...
volatile bool g_replay_track_flag = false;
volatile play_mode_t g_play_mode = PLAY_MODE_SEQUENTIAL;
volatile bool g_viz_enabled = false;
volatile int g_vgm_loop_count = 2;
...
volatile ay_stereo_mode_t g_ay_stereo_mode = AY_STEREO_ABC;
volatile cache_mode_t g_cache_mode = CACHE_MODE_UPDATE;
...
void save_configuration(int dev_idx, const char* slot0, const char* slot1, double speed, int flush_mode, int timer_mode, const char* last_file, int vgm_loop_count);
void clear_cache(const char* cache_path);
            ...
            if (stat(path, &st) == 0) {
                if (S_ISDIR(st.st_mode)) {
                    ...
                    if (ext && (strcasecmp(ext, ".vgm") == 0 || strcasecmp(ext, ".vgz") == 0 || strcasecmp(ext, ".s98") == 0)) {
                        strncpy(playlist[playlist_size], path, MAX_FILENAME_LEN - 1);
                        playlist[playlist_size][MAX_FILENAME_LEN - 1] = '\0';
                        playlist_size++;
                    }
                }
            }
...
int get_next_song_index() {
    if (g_play_mode == PLAY_MODE_RANDOM) {
        if (playlist_size > 1) {
            int next_index;
            do {
...
static int config_handler(void* user, const char* section, const char* name, const char* value, int lineno) {
    ...
    } else if (MATCH("playback", "vgm_loop_count")) {
        pconfig->vgm_loop_count = atoi(value);
    } else {
...
void save_configuration(int dev_idx, const char* slot0, const char* slot1, double speed, int flush_mode, int timer_mode, const char* last_file, int vgm_loop_count) {
    ...
    if (!file) {
        logging(LOG_LEVEL_ERROR, "Could not open %s for writing.\n", CONFIG_FILENAME);
        return;
    }
    fprintf(file, "[device]\n");
...
DWORD WINAPI keyboard_thread_func(LPVOID lpParam) {
    ...
    int c;
    while (!g_quit_flag) {
        // First check key queue (from viz windows)
        char queued_key;
        bool has_queued_key = key_queue_dequeue(&queued_key);
        ...
        if (has_queued_key) {
            ...
            switch (c) {
                case 72: // Arrow Up
                    g_opn_lfo_amplitude += 0.05;
                    if (g_opn_lfo_amplitude > 2.0) g_opn_lfo_amplitude = 2.0;
                    g_ui_refresh_request = true;
                    break;
                case 80: // Arrow Down
                    g_opn_lfo_amplitude -= 0.05;
                    if (g_opn_lfo_amplitude < 0.0) g_opn_lfo_amplitude = 0.0;
                    g_ui_refresh_request = true;
                    break;
                case 75: // Arrow Left
                    if (g_vgm_loop_count > 0) g_vgm_loop_count--;
                    g_ui_refresh_request = true;
                    break;
                case 77: // Arrow Right
                    g_vgm_loop_count++;
                ...
                case 'n': g_next_track_flag = true; g_stop_current_song = true; break;
                case 'b': g_prev_track_flag = true; g_stop_current_song = true; break;
                case 'r': g_replay_track_flag = true; g_stop_current_song = true; break;
                case 's': g_play_mode = (g_play_mode == PLAY_MODE_SEQUENTIAL) ? PLAY_MODE_RANDOM : PLAY_MODE_SEQUENTIAL; g_ui_refresh_request = true; break;
                ...
                case '\t': // Tab key
                    if (g_opn_to_opm_conversion_enabled && (g_original_vgm_chip_type == CHIP_TYPE_YM2203 || g_original_vgm_chip_type == CHIP_TYPE_YM2608)) {
                        ssg_to_opm_cycle_stereo_mode();
                    } else if (g_ay_to_opm_conversion_enabled) {
                        g_ay_stereo_mode = (g_ay_stereo_mode + 1) % AY_STEREO_MODE_COUNT;
                        ay_to_opm_set_stereo_mode(g_ay_stereo_mode);
                    }
                    g_ui_refresh_request = true;
                    break;
                case 'c':
                    g_cache_mode = (g_cache_mode == CACHE_MODE_NORMAL) ? CACHE_MODE_UPDATE : CACHE_MODE_NORMAL;
                    g_ui_refresh_request = true;
                    break;
                case 'v':
                case 'V':
                    // Cycle: None -> Slot0 -> Slot0+Slot1 -> Slot0 (cycle last 2 states)
                    // V key never closes Slot0, use window X button to close all
                    {
                        bool slot0_visible = saa1099_viz_is_slot_visible(0);
                        bool slot1_visible = saa1099_viz_is_slot_visible(1);

                        if (!slot0_visible && !slot1_visible) {
                            // State 1: Nothing open -> Open Slot0
                            printf("[V] Opening Slot0 visualization...\n");
                            saa1099_viz_set_slot_enabled(0, true);
                            saa1099_viz_set_slot_enabled(1, false);
                            saa1099_viz_start();
                            g_viz_enabled = true;
                        } else if (slot0_visible && !slot1_visible) {
                            // State 2: Only Slot0 -> Add Slot1
                            printf("[V] Adding Slot1 visualization...\n");
                            saa1099_viz_stop();
                            Sleep(50);  // Give time for cleanup
                            saa1099_viz_set_slot_enabled(0, true);
                            saa1099_viz_set_slot_enabled(1, true);
                            saa1099_viz_start();
                            g_viz_enabled = true;
                        } else if (slot0_visible && slot1_visible) {
                            // State 3: Both open -> Remove Slot1 (keep Slot0)
                            printf("[V] Removing Slot1 visualization...\n");
                            saa1099_viz_stop();
                            Sleep(50);  // Give time for cleanup
                            saa1099_viz_set_slot_enabled(0, true);
                            saa1099_viz_set_slot_enabled(1, false);
                            ...
                            // Edge case: Only Slot1 -> Reset to Slot0 only
                            printf("[V] Resetting to Slot0 visualization...\n");
                            saa1099_viz_stop();
                            Sleep(50);  // Give time for cleanup
                            saa1099_viz_set_slot_enabled(0, true);
                            saa1099_viz_set_slot_enabled(1, false);
                    ...
                    break;
                case '/':
                    // Cycle SAA1099 channel 0 mapping mode (None/Mute/Map to Slot0/Map to Slot1)
                    {
                        saa1099_ch0_map_mode_t current_mode = saa1099_get_ch0_map_mode();
                        saa1099_ch0_map_mode_t next_mode = (current_mode + 1) % SAA_CH0_MAP_MODE_COUNT;
                        saa1099_set_ch0_map_mode(next_mode);
                        g_ui_refresh_request = true;
                    }
                    break;
                case '<':
                case ',':
                    // Fast cycle SAA1099 mapping type (Tone <-> All only, skip Noise)
                    saa1099_cycle_map_type_fast();
                    g_ui_refresh_request = true;
                    break;
                case '>':
                case '.':
                    // Cycle SAA1099 target channel (Ch0-Ch5)
                    saa1099_cycle_map_target_channel();
                    g_ui_refresh_request = true;
                    break;
                case ';':
                    // Increase SAA1099 Ch0 repeat write count
                    saa1099_increase_ch0_repeat_count();
                    g_ui_refresh_request = true;
                    break;
                case '\'':
                    // Decrease SAA1099 Ch0 repeat write count
                    saa1099_decrease_ch0_repeat_count();
                    g_ui_refresh_request = true;
                    break;
                case '[':
                    // Decrease SAA1099 Ch0 delay samples
                    saa1099_decrease_ch0_delay_samples();
                    g_ui_refresh_request = true;
                    break;
                case ']':
                    // Increase SAA1099 Ch0 delay samples
                    saa1099_increase_ch0_delay_samples();
                    g_ui_refresh_request = true;
                    break;
            }
            LeaveCriticalSection(&g_playlist_lock);
        } else if (_kbhit()) {
            // No queued key, check console input
            c = _getch();
            if (c == 0 || c == 224) { // Arrow keys on Windows
                c = _getch();
            }

            // Enqueue the key so it's processed uniformly
            key_queue_enqueue((char)c);
        }
...
DWORD WINAPI player_thread_func(LPVOID lpParam) {
    ...
    while (!g_quit_flag) {
        ...
        if (replay_track) {
            EnterCriticalSection(&g_playlist_lock);
            // Don't change index, just set flags to re-trigger play
            g_is_playing = true;
            should_play = true;
            LeaveCriticalSection(&g_playlist_lock);
        }
        ...
        if (should_play) {
            ...
            save_configuration(dev_idx, slot0_name, slot1_name, g_speed_multiplier, g_flush_mode, g_timer_mode, g_current_song_name, g_vgm_loop_count);
            
            LeaveCriticalSection(&g_playlist_lock);

            spfm_chip_reset();
            bool force_reconvert = replay_track;
            if (replay_track) g_replay_track_flag = false; // Consume flag before playing

            play_file(playlist[current_song_index], force_reconvert);
            
            EnterCriticalSection(&g_playlist_lock);
            if (g_quit_flag) {
                g_is_playing = false;
            } else if (g_replay_track_flag || g_next_track_flag || g_prev_track_flag || g_new_song_request) {
                // A new request came in while playing. The loop will handle it.
                g_is_playing = false;
            } else {
...
void clear_cache(const char* cache_path) {
    ...
    if (!(dir = opendir(cache_path))) {
        logging(LOG_LEVEL_ERROR, "Could not open cache directory: %s", cache_path);
        return;
    }

    logging(LOG_LEVEL_INFO, "Clearing cache directory: %s", cache_path);
    while ((entry = readdir(dir)) != NULL) {
        ...
        if (remove(filepath) == 0) {
            logging(LOG_LEVEL_INFO, "Removed: %s", filepath);
        } else {
            logging(LOG_LEVEL_ERROR, "Failed to remove: %s", filepath);
        }
    }
    closedir(dir);
...
int main(int argc, char *argv[]) {
    ...
    // For simplicity, we'll just use the current working directory
    getcwd(exe_path, MAX_PATH_LEN);
#endif

    // Ensure cache and music directories exist
    char music_path[MAX_PATH_LEN + 16];
    char cache_path[MAX_PATH_LEN + 16];
    snprintf(music_path, sizeof(music_path), "%s/music", exe_path);
    snprintf(cache_path, sizeof(cache_path), "%s/cache", exe_path);
    ...
    g_vgm_loop_count = config.vgm_loop_count;

    ftStatus = FT_CreateDeviceInfoList(&numDevs);
    if (ftStatus != FT_OK) {
        logging(LOG_LEVEL_ERROR, "FT_CreateDeviceInfoList failed, error code: %d\n", (int)ftStatus);
        return 1;
    }

    if (numDevs == 0) {
        logging(LOG_LEVEL_ERROR, "No FTDI devices found.\n");
        return 1;
    }
    ...
    if (spfm_dev_count == 0) {
        logging(LOG_LEVEL_ERROR, "No SPFM devices found.\n");
        return 1;
    }
    ...
    if (spfm_dev_count > 1) {
        ...
        if (choice < 0 || choice >= spfm_dev_count) {
            logging(LOG_LEVEL_ERROR, "Invalid device selection.\n");
            return 1;
        }
    ...
    if (spfm_init(selected_dev_idx) != 0) {
        logging(LOG_LEVEL_ERROR, "Failed to initialize SPFM device.\n");
        return 1;
    }
    ...
    // Initialize chip configuration arrays
    for (int i = 0; i < CHIP_TYPE_COUNT; i++) {
        g_chip_config[i].type = (chip_type_t)i;
        g_chip_config[i].slot = 0xFF;
    }
    g_slot_to_chip[0] = CHIP_TYPE_NONE;
    g_slot_to_chip[1] = CHIP_TYPE_NONE;
    ...
    for (int slot = 0; slot < 2; slot++) {
        ...
        if (chip_choice > 0 && chip_choice < CHIP_TYPE_COUNT) {
            // Store in new slot-based array
            g_slot_to_chip[slot] = (chip_type_t)chip_choice;
            // Also update old array for backward compatibility (may overwrite if same chip in both slots)
            g_chip_config[chip_choice].slot = slot;
        } else {
            printf("Invalid choice or 0. Skipping slot.\n");
            g_slot_to_chip[slot] = CHIP_TYPE_NONE;
        }
    }

    // Get slot names from new array
    const char* slot0_name = chip_type_to_string(g_slot_to_chip[0]);
    const char* slot1_name = chip_type_to_string(g_slot_to_chip[1]);
    save_configuration(selected_dev_idx, slot0_name, slot1_name, g_speed_multiplier, g_flush_mode, g_timer_mode, config.last_file[0] ? config.last_file : NULL, g_vgm_loop_count);

    spfm_init_chips();

    // Initialize SAA1099 visualization
    saa1099_viz_init();

    // Check if SAA1099 is configured and mark slots as enabled
    for (int slot = 0; slot < 2; slot++) {
        if (g_slot_to_chip[slot] == CHIP_TYPE_SAA1099) {
            saa1099_viz_set_slot_enabled(slot, true);
        }
    }
    ...
    // From now on, redirect logging to file
    set_log_mode(LOG_TO_FILE);

    bool last_file_valid = false;
    if (config.last_file[0] != '\0') {
        FILE* f = fopen(config.last_file, "rb");
        if (f) {
            fclose(f);
            last_file_valid = true;
        } else {
            logging(LOG_LEVEL_INFO, "Last played file '%s' not found, rescanning.", config.last_file);
            config.last_file[0] = '\0'; // Invalidate it
        }
    }
    ...
    if (last_file_valid) {
        ...
        if (last_slash) {
            *last_slash = '\0';
            ...
            scan_music_directory("."); // Should not happen if path is absolute
        }
        ...
        if (current_song_index == -1) { // Song was in config, but not found in its own dir scan?
             last_file_valid = false; // Treat as invalid
             playlist_size = 0; // Clear playlist to force rescan
        }
    }
    
    if (!last_file_valid) {
        // No valid last file, scan default music directory
        scan_music_directory(music_path);
        if (playlist_size == 0) {
            // If music folder is empty, scan current directory as a fallback
            logging(LOG_LEVEL_INFO, "No music in 'music' folder, scanning current directory.");
    ...
    if (playlist_size == 0) {
        logging(LOG_LEVEL_INFO, "No music found, starting file browser.");
        g_app_mode = MODE_BROWSER;
        g_is_playing = false;
    } else {
        if (current_song_index < 0 || current_song_index >= playlist_size) {
            current_song_index = 0;
        }
        g_is_playing = true; // Start playing the first/selected song
    }
    ...
    while (!g_quit_flag) {
        if (g_app_mode == MODE_PLAYER) {
            if (g_ui_refresh_request) {
                update_ui(g_current_song_total_samples, g_current_song_name, g_is_paused, g_play_mode, g_ay_stereo_mode, g_cache_mode);
                g_ui_refresh_request = false;
            }
        } else { // MODE_BROWSER
            char browser_start_path[MAX_PATH_LEN];
            EnterCriticalSection(&g_playlist_lock);
            if (g_is_playing && playlist_size > 0 && current_song_index < playlist_size) {
                // Start browser in the directory of the current song
                strncpy(browser_start_path, playlist[current_song_index], MAX_PATH_LEN - 1);
                char* last_slash = strrchr(browser_start_path, '/');
                if (!last_slash) last_slash = strrchr(browser_start_path, '\\');
                if (last_slash) {
                    *last_slash = '\0';
                } else {
                    strcpy(browser_start_path, "."); // Fallback
                }
            } else {
                // Fallback to exe path if not playing
                strncpy(browser_start_path, exe_path, MAX_PATH_LEN - 1);
            }
            LeaveCriticalSection(&g_playlist_lock);

            const char* selected_file = file_browser(browser_start_path);
            if (selected_file) {
                EnterCriticalSection(&g_playlist_lock);
        ...
        /* This logic is redundant as the player_thread already handles advancing to the next track.
        if (!g_is_playing && !g_new_song_request && !g_next_track_flag && !g_prev_track_flag) {
            // If not in random mode, move to the next song
            if (g_play_mode == PLAY_MODE_SEQUENTIAL) {
                EnterCriticalSection(&g_playlist_lock);
                g_next_track_flag = true;
                LeaveCriticalSection(&g_playlist_lock);
            }
        }
        */
